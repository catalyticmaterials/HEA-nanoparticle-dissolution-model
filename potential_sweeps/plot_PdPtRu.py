...
data = np.loadtxt('PdPtRu_potential_sweep.csv',delimiter=',',skiprows=1)
...
print(np.median(np.std(Ucs,axis=1)))

U=np.max(Ucs,axis=1)


make_ternary_plot(grid,U,metals,vmin=np.min(U),vmax=np.max(U),colorbar=True,contour_levels=30,cbar_label='$U_c$ [V]',colormap='magma',maxval=0.95,minval=0.0,cbar_ticks=np.arange(0.3,1.1,0.1))
print(data[np.argmax(U),:3],np.max(U))
plt.savefig('PdPtRu_Ucrit.png',dpi=600,bbox_inches='tight')
...
fd_data = np.loadtxt('PdPtRu_potential_sweep_fd.csv',delimiter=',',skiprows=1)
...
for metal,ax in zip(['Ru','Pd','Pt'],axes):
    ...
    make_ternary_plot(grid,fd,['Pd','Pt','Ru'],ax=ax,vmin=0.0,vmax=1.0,colormap='coolwarm_r',minval=0.0)
...
plt.savefig(f'PdPtRu_Ucrit_fd.png',dpi=600,bbox_inches='tight')
...
fig,ax=make_ternary_plot(grid,fd,['Pd','Pt','Ru'],vmin=0.0,vmax=1.0,colormap='coolwarm_r',minval=0.0)
plt.colorbar(sm,ax=ax,shrink=0.5,anchor=(0.0,0.85),ticks=np.arange(0,1.1,0.2),label='$S_d$')

plt.savefig(f'PdPtRu_max_Ucrit_fd.png',dpi=600,bbox_inches='tight')