...
from utilities import all_metals, U_standard
...
with connect('metals_dissolution_out.db') as db, connect('../../databases_old/metals_dissolution_out.db') as db_old, connect('bulks.db') as db_bulk:
    ...
    for metal,marker in zip(metals,markers):
        ...
        if metal in ['Ag','Au','Cu']:
            ...
            no_defect = {row.surface:row.energy for row in db_old.select(metal=metal,defect='none')}
            add_defect = {row.surface:row.energy for row in db_old.select(metal=metal,defect='add')}

            remove_defect = {row.surface:row.energy for row in db_old.select(metal=metal,defect='removed')}
...
plt.tight_layout()
plt.savefig('metals_U.png',dpi=600,bbox_inches='tight')
...
for i,metal in enumerate(metals):
    ...
    ax.scatter(range(3,10),data_U.T[i],color=metal_colors[metal],label=metal)
...
plt.tight_layout()
plt.savefig('metals_U_comb.png',dpi=600,bbox_inches='tight')
