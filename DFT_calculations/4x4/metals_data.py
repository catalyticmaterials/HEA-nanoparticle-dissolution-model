...
with connect('metals_dissolution_4x4_out.db') as db, connect('../bulks.db') as db_bulk:

    E_bulk = {row.metal:row.energy for row in db_bulk.select()}


    for metal,marker in zip(metals,markers):
        if metal in ['Ir','Rh','Ru']:
            data_dE.append(np.empty(7))
            data_U.append(np.empty(7))
            continue
        ...
        ax1.scatter(CN,dE, color=metal_colors[metal],marker=marker)

        U_metal = U_standard[metal]
        
        U = dE/U_metal['n'] + U_metal['U']

        ax2.scatter(CN,U, color=metal_colors[metal],marker=marker)
...
print(data_dE[3,[0,1,2,4,5]])
print(np.mean(np.abs(data_dE[3,[0,1,2,4,5]])))